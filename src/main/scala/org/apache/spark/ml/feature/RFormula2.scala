...
/**
  * Base trait for [[RFormula2]] and [[RFormulaModel2]].
  */
private[feature] trait RFormulaBase2 extends HasFeaturesCol with HasLabelCol {
  ...
  @Since("original")
  val encodeColumnName: BooleanParam = new BooleanParam(this, "encodeColumnName", "")
  setDefault(encodeColumnName, false)
...
@Experimental
@Since("1.5.0")
class RFormula2 @Since("1.5.0") (@Since("1.5.0") override val uid: String)
  extends Estimator[RFormulaModel2] with RFormulaBase2 with DefaultParamsWritable {
  ...
  @Since("original")
  def setEncodeColumnName(value: Boolean): this.type = set(encodeColumnName, value)
  ...
  @Since("2.0.0")
  override def fit(dataset: Dataset[_]): RFormulaModel2 = {
    ...
    val indexed: Map[String, String] = resolvedFormula.terms.flatten.distinct.map { term =>
      dataset.schema(term) match {
        case column if column.dataType == StringType =>
          ...
          prefixesToRewrite(indexCol + "_") = (if ($(encodeColumnName)) Base64.getEncoder.encodeToString(term.getBytes) else term) + "_"
          (term, indexCol)
        case _ =>
          (term, term)
      }
    ...
    val encodedTerms = resolvedFormula.terms.map {
      case Seq(term) if dataset.schema(term).dataType == StringType =>
        ...
        prefixesToRewrite(encodedCol + "_") = (if ($(encodeColumnName)) Base64.getEncoder.encodeToString(term.getBytes) else term) + "_"
        encodedCol
      case Seq(term) =>
        term
      case terms =>
    ...
    encoderStages += new VectorAttributeRewriter2($(featuresCol), prefixesToRewrite.toMap)
    encoderStages += new ColumnPruner2(tempColumns.toSet)
    ...
    copyValues(new RFormulaModel2(uid, resolvedFormula, pipelineModel).setParent(this))
  }
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): RFormula2 = defaultCopy(extra)
...
@Since("2.0.0")
object RFormula2 extends DefaultParamsReadable[RFormula2] {

  @Since("2.0.0")
  override def load(path: String): RFormula2 = super.load(path)
}

/**
  * :: Experimental ::
  * Model fitted by [[RFormula2]]. Fitting is required to determine the factor levels of
  * formula terms.
  *
...
@Experimental
@Since("1.5.0")
class RFormulaModel2 private[feature](
  @Since("1.5.0") override val uid: String,
  private[ml] val resolvedFormula: ResolvedRFormula,
  private[ml] val pipelineModel: PipelineModel)
  extends Model[RFormulaModel2] with RFormulaBase2 with MLWritable {
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): RFormulaModel2 = {
    val copied = new RFormulaModel2(uid, resolvedFormula, pipelineModel).setParent(parent)
  ...
  @Since("2.0.0")
  override def write: MLWriter = new RFormulaModel2.RFormulaModelWriter(this)
}

@Since("2.0.0")
object RFormulaModel2 extends MLReadable[RFormulaModel2] {

  @Since("2.0.0")
  override def read: MLReader[RFormulaModel2] = new RFormulaModelReader

  @Since("2.0.0")
  override def load(path: String): RFormulaModel2 = super.load(path)

  /** [[MLWriter]] instance for [[RFormulaModel2]] */
  private[RFormulaModel2] class RFormulaModelWriter(instance: RFormulaModel2) extends MLWriter {
  ...
  private class RFormulaModelReader extends MLReader[RFormulaModel2] {

    /** Checked against metadata when loading model */
    private val className = classOf[RFormulaModel2].getName

    override def load(path: String): RFormulaModel2 = {
      ...
      val model = new RFormulaModel2(metadata.uid, resolvedRFormula, pipelineModel)
...
/**
  ...
  */
private class ColumnPruner2(override val uid: String, val columnsToPrune: Set[String])
  ...
  override def copy(extra: ParamMap): ColumnPruner2 = defaultCopy(extra)

  override def write: MLWriter = new ColumnPruner2.ColumnPrunerWriter(this)
}

private object ColumnPruner2 extends MLReadable[ColumnPruner2] {

  override def read: MLReader[ColumnPruner2] = new ColumnPrunerReader

  override def load(path: String): ColumnPruner2 = super.load(path)

  /** [[MLWriter]] instance for [[ColumnPruner2]] */
  private[ColumnPruner2] class ColumnPrunerWriter(instance: ColumnPruner2) extends MLWriter {
  ...
  private class ColumnPrunerReader extends MLReader[ColumnPruner2] {

    /** Checked against metadata when loading model */
    private val className = classOf[ColumnPruner2].getName

    override def load(path: String): ColumnPruner2 = {
      val metadata = DefaultParamsReader.loadMetadata(path, sc, className)

      val dataPath = new Path(path, "data").toString
      val data = sparkSession.read.parquet(dataPath).select("columnsToPrune").head()
      val columnsToPrune = data.getAs[Seq[String]](0).toSet
      val pruner = new ColumnPruner2(metadata.uid, columnsToPrune)
...
/**
  ...
  */
private class VectorAttributeRewriter2(
  ...
  override def copy(extra: ParamMap): VectorAttributeRewriter2 = defaultCopy(extra)

  override def write: MLWriter = new VectorAttributeRewriter2.VectorAttributeRewriterWriter(this)
}

private object VectorAttributeRewriter2 extends MLReadable[VectorAttributeRewriter2] {

  override def read: MLReader[VectorAttributeRewriter2] = new VectorAttributeRewriterReader2

  override def load(path: String): VectorAttributeRewriter2 = super.load(path)

  /** [[MLWriter]] instance for [[VectorAttributeRewriter2]] */
  private[VectorAttributeRewriter2]
  class VectorAttributeRewriterWriter(instance: VectorAttributeRewriter2) extends MLWriter {
  ...
  private class VectorAttributeRewriterReader2 extends MLReader[VectorAttributeRewriter2] {

    /** Checked against metadata when loading model */
    private val className = classOf[VectorAttributeRewriter2].getName

    override def load(path: String): VectorAttributeRewriter2 = {
      val metadata = DefaultParamsReader.loadMetadata(path, sc, className)
      ...
      val rewriter = new VectorAttributeRewriter2(metadata.uid, vectorCol, prefixesToRewrite)
